...
//cojemos el intrumento por query y hacemos una busqueda de los usuarios con esa categoria sin incluir al usuario de la sessión y renderizamos la lista de profesores de ese intrumento
  try {
...
router.get('/detail/:id', requireUser, uploadCloud.single('image-back'), uploadCloud.single('image-perfil'), async (req, res, next) => {
  const { id } = req.params
  //recojemos por parametro la id del profesor que clickemos, lo buscamos por su id y mostramos su perfil
  try {
...
//creamos el match entre el profesor y el alumno
...
// Matches, vemos los matches en nuestro perfil
router.get('/profile', requireUser, uploadCloud.single('image-perfil'), async (req, res, next) => {
  const _id = req.session.currentUser._id
  // const { id } = req.params
//buscamos los matches que tenemos de profesores y de alumnos y los renderizamos
  try {
